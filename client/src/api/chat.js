import { toast } from 'react-toastify';
import { apiService } from './apiService';

export const acessCreate = async (body) => {
    try {
      const { data } = await apiService.post('/api/chat', body);
      console.log(data);
      return data;
    } catch (error) {
      console.log('error in access create api');
    }
  };
  export const fetchAllChats = async () => {
    try {
      const { data } = await apiService.get('/api/chat');
      return data;
    } catch (error) {
      console.log('error in fetch all chats api');
    }
  };
  export const createGroup = async (body) => {
    try {
      const { data } = await apiService.post('/api/chat/group', body);
      toast.success(`${data.chatName} Group Created`);
      return data;
    } catch (error) {
      console.log('error in create group api');
    }
  };
  export const addToGroup = async (body) => {
    try {
      const { data } = await apiService.post('/api/chat/groupAdd', body);
      return data;
    } catch (error) {
      console.log('error in addtogroup api');
    }
  };
  export const renameGroup = async (body) => {
    try {
      const { data } = await apiService.post('/api/chat/group/rename', body);
      return data;
    } catch (error) {
      console.log('error in rename group api');
    }
  };
  export const removeUser = async (body) => {
    try {
      const { data } = await apiService.post('/api/chat/groupRemove', body);
      return data;
    } catch (error) {
      console.log('error in remove user api');
    }
  };