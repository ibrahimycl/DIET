export const isSameSenderMargin = (messages, m, i, userId) => {
  ...
  export const getChatName = (activeChat, activeUser) => {
    return activeChat[0]?.isGroup
      ? activeChat[0]?.chatName
      : activeChat[0]?.users[0]?._id == activeUser._id
      ? activeChat[0]?.users[1]?.name
      : activeChat[0]?.users[0]?.name;
  };
  export const getChatPhoto = (activeChat, activeUser) => {
    return activeChat[0]?.isGroup
      ? activeChat[0].photo
      : activeChat[0]?.users[0]?._id === activeUser?.id
      ? activeChat[0]?.users[1]?.profilePic
      : activeChat[0]?.users[0]?.profilePic;
  };